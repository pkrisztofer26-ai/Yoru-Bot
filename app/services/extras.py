...
class ExtrasService:
    ...
    async def weekly(self, guild_id: int, user_id: int) -> tuple[int, datetime]:
        ...
        now = await self._cooldown(guild_id, user_id, "last_weekly", self.WEEKLY_COOLDOWN)
        reward = random.randint(*eco.WEEKLY_REWARD)
        ...
        return reward, now + self.WEEKLY_COOLDOWN
    ...
    async def monthly(self, guild_id: int, user_id: int) -> tuple[int, datetime]:
        ...
        now = await self._cooldown(guild_id, user_id, "last_monthly", self.MONTHLY_COOLDOWN)
        reward = random.randint(*eco.MONTHLY_REWARD)
        ...
        return reward, now + self.MONTHLY_COOLDOWN

    async def interest(self, guild_id: int, user_id: int) -> tuple[int, int, datetime]:
        now = await self._cooldown(guild_id, user_id, "last_interest", self.INTEREST_COOLDOWN)
        _, bank = await self.db.get_balance(guild_id, user_id)
        if bank < eco.INTEREST_MIN_BANK:
            raise ValueError(f"Legalább ${eco.INTEREST_MIN_BANK:,} kell a bankodban a kamathoz.".replace(",", " "))
        ...
        return reward, new_bank, now + self.INTEREST_COOLDOWN
    ...
    async def chickenfight(self, guild_id: int, user_id: int, bet: int) -> tuple[bool, str, int, int]:
        ...
        if bet < eco.GAMBLING_MIN_BET:
            raise ValueError(f"A minimum tét ${eco.GAMBLING_MIN_BET:,}.".replace(",", " "))
        ...
        profit = int(bet * (eco.CHICKEN_TOTAL_PAYOUT - 1.0)) if won else -bet
    ...
    async def highlow(self, guild_id: int, user_id: int, choice: str, bet: int) -> tuple[int, str, int, int]:
        ...
        profit = int(bet * (eco.HIGHLOW_TOTAL_PAYOUT - 1.0)) if won else -bet
    ...
    async def rps(self, guild_id: int, user_id: int, choice: str, bet: int) -> tuple[str, str, int, int]:
        ...
        profit = int(bet * (eco.RPS_TOTAL_PAYOUT - 1.0)) if won else -bet
