...
DEFAULT_LICENSE_PRICE = 5_000_000
...
MAX_REPUTATION = 1000
PORTFOLIO_REPUTATION_PER_EXTRA_PROPERTY = 500

BUSINESS_LEVEL_NAMES = {
    1: "Alap működés",
    2: "Bővített működés",
    3: "Jól kiépített",
    4: "Fejlett működés",
    5: "Kiemelt vállalkozás",
}


def reputation_name(value: int) -> str:
    value = max(0, int(value))
    if value < 80:
        return "Új vállalkozás"
    if value < 240:
        return "Feltörekvő"
    if value < 500:
        return "Helyben ismert"
    if value < 800:
        return "Elismert"
    return "Országosan ismert"


def business_level_name(level: int) -> str:
    return BUSINESS_LEVEL_NAMES.get(max(1, min(MAX_BUSINESS_LEVEL, int(level))), "Alap működés")


_UPGRADE_NAMES = {
    "Vendéglátás": ("Konyha és kiszolgálótér bővítése", "Beszerzés korszerűsítése", "Prémium vendégtér kialakítása", "Második műszak bevezetése"),
    "Kereskedelem": ("Raktár bővítése", "Beszerzési rendszer fejlesztése", "Üzlettér korszerűsítése", "Országos értékesítési háttér"),
    "Szolgáltatás": ("Műhely és eszközpark bővítése", "Ügyfélkezelés korszerűsítése", "Prémium szolgáltatási csomag", "Nagyobb kapacitás kiépítése"),
    "Szórakozás": ("Vendégtér bővítése", "Technikai háttér fejlesztése", "Prémium programkínálat", "Nagyobb rendezvénykapacitás"),
    "Technológia": ("Munkaállomások bővítése", "Infrastruktúra korszerűsítése", "Szakmai csapat bővítése", "Nagyvállalati kapacitás kiépítése"),
    "Logisztika": ("Raktárkapacitás bővítése", "Járműpark fejlesztése", "Diszpécserközpont korszerűsítése", "Országos logisztikai háttér"),
    "Rendezvény": ("Helyszín bővítése", "Technikai eszközpark fejlesztése", "Prémium rendezvénycsomag", "Nagyrendezvény-kapacitás"),
    "Turizmus": ("Szobakapacitás bővítése", "Vendégszolgáltatások fejlesztése", "Prémium részleg kialakítása", "Kiemelt turisztikai kapacitás"),
    "Média": ("Stúdió bővítése", "Technikai háttér korszerűsítése", "Produkciós csapat bővítése", "Országos produkciós kapacitás"),
}


def upgrade_name(category: str, current_level: int) -> str:
    names = _UPGRADE_NAMES.get(str(category), (
        "Működés bővítése", "Eszközpark fejlesztése", "Kapacitás növelése", "Kiemelt működés kiépítése"
    ))
    index = max(0, min(len(names) - 1, int(current_level) - 1))
    return names[index]
...
WORKERS: tuple[WorkerDefinition, ...] = (
    ...
    WorkerDefinition("lili", "Lili • Közösségi menedzser", "📱", "Szakértő", 18, 4_500_000, 38_000),
    ...
    WorkerDefinition("zsofi", "Zsófi • Arculattervező", "🎨", "Haladó", 9, 1_600_000, 14_000),
    WorkerDefinition("bence", "Bence • Logisztikus", "📦", "Haladó", 8, 1_450_000, 13_000),
    WorkerDefinition("eszter", "Eszter • Házigazda", "✨", "Profi", 11, 2_250_000, 20_000),
    WorkerDefinition("marci", "Marci • Tanonc", "🧑‍🔧", "Tanonc", 5, 650_000, 7_000),
)
