...
assert manifest["human_qa_regressions"] == "5/5 ENCODED"
assert manifest["replay_effective_human_qa"] == "14/14 GO"
assert manifest["fresh_v3836_groq_rerun"] == "QUEUED_NOT_COUNTED_AS_COMPLETED"
...
for required in (
    ...
    "deterministic_context_fallback", "asyncio.wait_for", "tier2-context-surface-v3",
):
    assert required in layer, required

for regression in (
    "otthonvárosban\\s+helyezkedik", "ügyben\\s+áll", "érkezett\\b.{0,48}\\bérkezett",
    "szolgáltatás\\s+profilú", "szívességet\\s+tart\\s+nyilván",
):
...
# Compatibility line for the already-queued proven W22.4.1 workflow shell gate.
...
print("HUMAN_QA_REGRESSIONS=5/5 ENCODED")
