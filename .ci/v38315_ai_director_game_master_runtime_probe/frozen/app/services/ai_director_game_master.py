...
class AIDirectorGameMaster:
    """W22.7 observed fail-closed Tier 3 test-guild presentation service.
    ...
    def note_presentation_result(self, *, added: bool = False, integration_failure: bool = False) -> None:
        if added:
    ...
    async def surface(self, guild_id: int | None, packet: AIDirectorGameMasterPacket) -> AIDirectorGameMasterSurface | None:
        ...
        try:
            ...
            self._record("deterministic_fallbacks")
            logger.warning("Tier3 Game Master timeout fallback guild=%s family=%s key=%s error=%s", guild_id, packet.family, packet.story_key, type(exc).__name__)
            return fallback
        except AIDirectorGameMasterValidationError as exc:
            self._record("output_validation_failures")
            self._record("deterministic_fallbacks")
            logger.warning("Tier3 Game Master validation fallback guild=%s family=%s key=%s error=%s", guild_id, packet.family, packet.story_key, type(exc).__name__)
            return fallback
