...
class Settings:
    ...
    ai_pilot_guild_id: int | None
    ai_director_pilot_enabled: bool
    ai_director_context_pilot_enabled: bool
    ai_director_game_master_pilot_enabled: bool
    ai_director_game_master_rollout_enabled: bool
    ai_director_game_master_rollout_guild_id: int | None
    ai_director_game_master_rollout_percent: int
...
def load_settings() -> Settings:
    ...
    ai_pilot_guild_id = _optional_snowflake("YORU_AI_PILOT_GUILD_ID")
    ...
    ai_director_game_master_rollout_enabled = env_bool("YORU_AI_DIRECTOR_GAME_MASTER_ROLLOUT_ENABLED", False)
    ai_director_game_master_rollout_guild_id = _optional_snowflake("YORU_AI_GAME_MASTER_ROLLOUT_GUILD_ID")
    ai_director_game_master_rollout_percent = _env_int("YORU_AI_GAME_MASTER_ROLLOUT_PERCENT", 0)
    groq_api_key = os.getenv("GROQ_API_KEY", "").strip()
    if ai_director_pilot_enabled and ai_pilot_guild_id is None:
        raise RuntimeError(
            "A YORU_AI_DIRECTOR_PILOT_ENABLED csak explicit YORU_AI_PILOT_GUILD_ID mellett kapcsolható be."
        )
    if ai_director_context_pilot_enabled:
        if not ai_director_pilot_enabled:
            raise RuntimeError("A Tier 2 Context Pilot csak bekapcsolt Tier 1 AI pilot mellett engedélyezhető.")
        if ai_pilot_guild_id is None:
            raise RuntimeError("A YORU_AI_DIRECTOR_CONTEXT_PILOT_ENABLED csak explicit YORU_AI_PILOT_GUILD_ID mellett kapcsolható be.")
        if not groq_api_key:
    ...
    if ai_director_game_master_pilot_enabled:
        ...
        if ai_pilot_guild_id is None:
            raise RuntimeError("A YORU_AI_DIRECTOR_GAME_MASTER_PILOT_ENABLED csak explicit YORU_AI_PILOT_GUILD_ID mellett kapcsolható be.")
        if not groq_api_key:
            raise RuntimeError("A Tier 3 Game Master Pilot bekapcsolásához GROQ_API_KEY szükséges.")
    if ai_director_game_master_rollout_percent < 0 or ai_director_game_master_rollout_percent > 10:
        raise RuntimeError("A YORU_AI_GAME_MASTER_ROLLOUT_PERCENT 0 és 10 között lehet.")
    if ai_director_game_master_rollout_enabled:
        if not ai_director_pilot_enabled or not ai_director_game_master_pilot_enabled:
            raise RuntimeError("A W22.8 rollout csak bekapcsolt Tier 1 + Tier 3 pilot mellett engedélyezhető.")
        if ai_director_game_master_rollout_guild_id is None:
            raise RuntimeError("A W22.8 rollout csak explicit YORU_AI_GAME_MASTER_ROLLOUT_GUILD_ID mellett kapcsolható be.")
        if ai_director_game_master_rollout_percent <= 0:
            raise RuntimeError("Bekapcsolt W22.8 rollout mellett a YORU_AI_GAME_MASTER_ROLLOUT_PERCENT legyen 1-10.")
        if ai_pilot_guild_id is not None and ai_director_game_master_rollout_guild_id == ai_pilot_guild_id:
            raise RuntimeError("A W22.8 rollout guild nem lehet azonos az izolált YORU_AI_PILOT_GUILD_ID szerverrel.")
        if not groq_api_key:
            raise RuntimeError("A W22.8 Tier 3 rollout bekapcsolásához GROQ_API_KEY szükséges.")
    ...
    return Settings(
        ...
        ai_pilot_guild_id=ai_pilot_guild_id,
        ...
        ai_director_game_master_rollout_enabled=ai_director_game_master_rollout_enabled,
        ai_director_game_master_rollout_guild_id=ai_director_game_master_rollout_guild_id,
        ai_director_game_master_rollout_percent=ai_director_game_master_rollout_percent,
